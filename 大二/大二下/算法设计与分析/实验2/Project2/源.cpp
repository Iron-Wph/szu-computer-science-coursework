...
// 合并两个有序数组
void merge_sort(point* arr, int left, int mid, int right) {
    int n1 = mid - left + 1;    // 左侧数组的大小
    int n2 = right - mid;       // 右侧数组的大小

    // 创建临时数组
    point* temp1 = new point[n1];
    point* temp2 = new point[n2];
...
// 合并
double merge(point* arr, int low, int mid, int high, double mind)
{
    // 存入中线附近+-最短距离的点
    int size = high - low + 1;
    ...
    size = k;
    // 对区域内的点集按照y轴坐标排序
    //Qsort(temp, 0, size-1);	// 快速排序
    merge_sort(temp, 0, size >> 1, size - 1);	//归并排序
    //sort(temp,temp+size,comp);
...
double fenzhi(point* arr, int low, int high)
{
    // 只有一个点
    if (low == high)
    {
...
{   
    ...
    for (int i = 10; i <= 10; i++)
    {
        ...
        for (int j = 1; j <= 5; j++)
        {
            ...
            //分治法
            sort(point_set, point_set + size, com);
            fenzhi(point_set, 0, size );
