...
// 合并
double merge(vector<point>&arr,int low,int mid,int high,double mind, point *ans)
{
	// 存入中线附近+-最短距离的点
	vector<point>temp;
...
// 分治法求解
double fenzhi(vector<point> &arr,int low,int high, point *ans)
{
	// 只有一个点
	if(low == high)
	{
	...
	{
		double t = dist(arr[low],arr[high]);
		if(t < dist(ans[0],ans[1]))
		{
			ans[0] = arr[low];
			ans[1] = arr[high];
		} 
	...
	else
	{
		// 取出中间点二分
		int mid = (low+high) >> 1;
		double dl = fenzhi(arr,low,mid,ans);
		double dr = fenzhi(arr,mid+1,high,ans);
		double mindist = dl<dr ? dl:dr;
		// 合并
		double t = merge(arr,low,mid,high,mindist,ans);
...
int main()
{
    ...
    for (int i = 1; i <= 10; i++)
    {
        ...
        for (int j = 1; j <= 10; j++)
        {
            ...
            double t = fenzhi(point_set,0,point_set.size()-1,ans);            
			...
			// 输出分治法的求解结果
			cout<<t<<"  坐标："<< ans[0].x<<" "<<ans[0].y<<"  "<<ans[1].x<<" "<<ans[1].y<<endl;
