...
def nnCostFunction(nn_params, input_layer_size, hidden_layer_size, num_labels, X, y, _lambda):
    # 隐藏层计算的参数，加上偏置，为25*401
    Theta1 = nn_params[0:hidden_layer_size * (input_layer_size + 1)].reshape(hidden_layer_size, input_layer_size + 1)
    # 输出层计算的参数，加上偏置，为10*26
    ...
    #2.计算代价函数
    # 无正则化
    J = (-1/m) * np.sum(y_onehot*np.log(h)+(1-y_onehot)*np.log(1-h))

    # 加上正则化项
    J += (_lambda/(2*m)) * (np.sum(Theta1[:, 1:]*Theta1[:, 1:]) + np.sum(Theta2[:, 1:]*Theta2[:, 1:]))

    #3.反向传播获取梯度
    # 对每个样本计算误差
    # for t in range(m):
    # 第三层的误差为a3-y，为5000*10
    e3 = h - y_onehot
    # 第二层的误差，为5000*25
    e2 = e3 @ Theta2[:, 1:Theta2.shape[1]] * (sigmoid(z2) * (1 - sigmoid(z2)))
