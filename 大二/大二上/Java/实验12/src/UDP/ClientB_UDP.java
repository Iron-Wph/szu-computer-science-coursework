...
//控制接受信息的线程
class InputThreadB_UDP extends Thread
{
    ...
    // 传入当前客户端的套接字
    InputThreadB_UDP(DatagramSocket tSocket) throws IOException {
        //根据要接受对象的套接字，初始化数据包
    ...
    {   //持续监听服务器转发的信息
        while(true)
        {
            ...
            try
            {
                ...
                System.out.println("ClientA:"+str);
                // 线程休眠0.1s进行缓冲
                Thread.sleep(100);
            }
            catch (IOException e)
...
//控制发送信息的线程
class OutputThreadB_UDP extends Thread
{
    // 数据存储的空间，数据包data以及当前代表当前客户端的套接字
    byte b[] = new byte[1000];
    DatagramPacket data;
    DatagramSocket mail_out;
    OutputThreadB_UDP(int dis, DatagramSocket tSocket) throws IOException {
        //构建向服务器发送信息的输出流
...
public class ClientB_UDP
{
    public static void main(String args[])
    {
        try
        {
            ...
            InputThreadB_UDP in = new InputThreadB_UDP(SocketB);
            // 发送信息的线程应该传入服务端的接口portC
            OutputThreadB_UDP out = new OutputThreadB_UDP(portC,SocketB);
