            ...
            if len(faces) != 0:
                ...
                for k in range(0, 100):
                    ...
                    if mindist > dispt:
                        ...
                        label = k
                    print("距离：", dispt)
                final_label = labels[label] - 1

                print("min dispt :", mindist)
                print("label num:", final_label)
                # 输出分类结果
                print(f"label:{foldername[final_label]}")
                # 如果分类正确，分类正确结果加1
                if final_label == j:
                    detect += 1

                # 在图像上输出标签
                label_text = f"{foldername[final_label]}"
...
if __name__ == '__main__':
    ...
    predict(train_Set_reduce, train_Set_EigVects, mean_train_Set)
