...
void init()
{
	...
	cube->readObj("./assets/cube/cube2.obj");
	// 设置mtl格式
	std::ifstream fin("./assets/cube/cube2.mtl");
	std::string line;
	while (std::getline(fin, line)) {
		...
		if (type == "Ns") {
			sin >> Ns;
			cube->setShininess(Ns);
		}
		if (type == "Ka") {
			sin >> Ka0 >> Ka1 >> Ka2;
			cube->setAmbient(glm::vec4(Ka0, Ka1, Ka2, 1.0f));
		}
		if (type == "Kd") {
			sin >> Kd0 >> Kd1 >> Kd2;
			cube->setDiffuse(glm::vec4(Kd0, Kd1, Kd2, 1.0f));
		}
		if (type == "Ks") {
			sin >> Ks0 >> Ks1 >> Ks2;
			cube->setSpecular(glm::vec4(Ks0, Ks1, Ks2, 1.0f));
		}
	}
	// 设置物体的旋转位移
	cube->setTranslation(glm::vec3(0.0, 0.0, 0.0));
	cube->setRotation(glm::vec3(-90.0, 0.0, 0.0));
	cube->setScale(glm::vec3(0.8, 0.8, 0.8));
	// 加到painter中
	painter->addMesh(cube, "mesh_c", "./assets/cube/wall.jpg", vshader, fshader); 	// 指定纹理与着色器
	// 程序结束时回收物体数据 
...
int main(int argc, char** argv)
{
	...
	// 配置窗口属性
	GLFWwindow* window = glfwCreateWindow(600, 600, "2022152021_王培鸿_实验4", NULL, NULL);
	if (window == NULL)
	{
