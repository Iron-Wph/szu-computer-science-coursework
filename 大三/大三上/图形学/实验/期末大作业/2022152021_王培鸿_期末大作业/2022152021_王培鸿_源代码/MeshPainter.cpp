...
void MeshPainter::addMesh(TriMesh* mesh, const std::string &name, const std::string &texture_image, const std::string &vshader, const std::string &fshader, bool shadow){
...
void MeshPainter::drawMesh(TriMesh* mesh, openGLObject &object, Light *light, Camera* camera, bool projMode, bool shadow){
    ...
    if (shadow)
    {
        ...
        // 传递 unifrom 关键字的矩阵数据。
        glUniformMatrix4fv(object.modelLocation, 1, GL_FALSE, &model[0][0]);
        glUniformMatrix4fv(object.viewLocation, 1, GL_FALSE, &camera->viewMatrix[0][0]);
        glUniformMatrix4fv(object.projectionLocation, 1, GL_FALSE, &camera->projMatrix[0][0]);
        // 绘制
        glDrawArrays(GL_TRIANGLES, 0, mesh->getPoints().size());
...
void MeshPainter::drawMeshes(Light *light,Camera *camera, bool projMode){
    for (int i = 0; i < meshes.size(); i++)
    {
        drawMesh(meshes[i], opengl_objects[i], light, camera, projMode, shadows[i]);
    }
};
