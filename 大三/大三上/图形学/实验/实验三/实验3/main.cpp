...
void init()
{
	...
	// 设置光源位置，使得光源位于物体的正上方
	light->setTranslation(glm::vec3(0.0, 3.0, 0));
	light->setAmbient(glm::vec4(1.0, 1.0, 1.0, 1.0)); // 环境光
	light->setDiffuse(glm::vec4(1.0, 1.0, 1.0, 1.0)); // 漫反射
	light->setSpecular(glm::vec4(1.0, 1.0, 1.0, 1.0)); // 镜面反射

	// 设置物体的旋转位移
	mesh->setTranslation(glm::vec3(0.0, 0.62, 0.0));
	...
	plane->setAmbient(glm::vec4(0.135f, 0.2225f, 0.1575f, 0.95f)); // 环境光
	plane->setDiffuse(glm::vec4(0.54f, 0.89f, 0.63f, 0.95f)); // 漫反射
	plane ->setSpecular(glm::vec4(0.316228f, 0.316228f, 0.316228f, 0.95f)); // 镜面反射
	plane->setShininess(12.8f); //高光系数
...
void display()
{
	...
	model = light->getShadowProjectionMatrix() * model;
	glUniformMatrix4fv(mesh_object.modelLocation, 1, GL_FALSE, &model[0][0]);
	glUniformMatrix4fv(mesh_object.viewLocation, 1, GL_FALSE, &camera->viewMatrix[0][0]);
	glUniformMatrix4fv(mesh_object.projectionLocation, 1, GL_FALSE, &camera->projMatrix[0][0]);
	// 绘制阴影
	...
	glm::mat4 Matrix = plane->getModelMatrix();
	glUniformMatrix4fv(plane_object.modelLocation, 1, GL_FALSE, &Matrix[0][0]);
...
void mainWindow_key_callback(GLFWwindow* window, int key, int scancode, int action, int mode)
{
	...
	else if (key == GLFW_KEY_0 && action == GLFW_PRESS && mode == 0x0000)
	{
		...
		tmp = shininess;
		shininess = std::min(tmp + 10.0, 100.0);
	...
	else if (key == GLFW_KEY_0 && action == GLFW_PRESS && mode == GLFW_MOD_SHIFT)
	{
		shininess = mesh->getShininess();
		tmp = shininess;
		shininess = std::max(tmp - 10.0, 0.0);
		mesh->setShininess(shininess);
	}
...
int main(int argc, char **argv)
{
	...
	GLFWwindow* mainwindow = glfwCreateWindow(600, 600, "2022152021_王培鸿_实验3", NULL, NULL);
	if (mainwindow == NULL)
	{
