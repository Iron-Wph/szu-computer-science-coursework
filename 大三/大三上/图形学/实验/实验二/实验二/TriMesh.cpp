...
void TriMesh::storeFacesPoints() {
    ...
    for (int i = 0; i < faces.size(); i++)
    {
        ...
        // 颜色
        colors.push_back(3.0f * vertex_colors[face.x] + vertex_colors[face.y] + vertex_colors[face.z]);
        colors.push_back(3.0f * vertex_colors[face.y] + vertex_colors[face.z] + vertex_colors[face.x]);
        colors.push_back(3.0f * vertex_colors[face.z] + vertex_colors[face.x] + vertex_colors[face.y]);
    }
}
...
void TriMesh::generateCube() {
    ...
    faces.push_back(vec3i(5, 3, 7));
    // 后面
    faces.push_back(vec3i(4, 0, 2));
    faces.push_back(vec3i(4, 2, 6));
    // 左面
    faces.push_back(vec3i(1, 0, 4));
    faces.push_back(vec3i(5, 1, 4));
    // 右面
    faces.push_back(vec3i(7, 3, 6));
    faces.push_back(vec3i(3, 2, 6));
